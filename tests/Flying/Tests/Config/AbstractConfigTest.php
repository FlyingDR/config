...
class AbstractConfigTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $object = $this->getConfigObject();
        $this->validateConfig($object->getConfig(), $this->_configReference, true);
    }
    ...
    {
        $object = $this->getConfigObject();
        $this->validateConfig($object->getConfig(null), $this->_configReference, true);
        $this->validateConfig($object->getConfig(true), $this->_configReference, true);
        $this->validateConfig($object->getConfig(false), $this->_configReference, true);
        $this->validateConfig($object->getConfig(array()), $this->_configReference, true);
        $this->validateConfig($object->getConfig(new \ArrayObject()), $this->_configReference, true);
    }
    ...
    protected function runConfigModificationTest($modifications, $reference)
    {
        ...
        $this->validateConfig($config, $reference, true);
    }
    ...
    public function testSettingMultipleConfigOptions()
    {
        ...
        $this->validateConfig($object->getConfig(), $this->_configSetReference, true);
    }
     ...
     * @param boolean $full     OPTIONAL TRUE to perform full testing of all configuration options
     * @return void
     */
    protected function validateConfig($received, $expected, $full = true)
    {
        $expected[ConfigurableInterface::CLASS_ID_KEY] = get_class($this->getConfigObject());
